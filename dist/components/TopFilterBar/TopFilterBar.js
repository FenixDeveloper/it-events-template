...
var TopFilterBar = function TopFilterBar() {
  ...
  var deleteValue = function deleteValue(item, tag) {
    if (item === "status" || item === "tags" || item === "specialities") {
      var updatedValues = _objectSpread({}, values);
      updatedValues[item] = values[item].filter(function (value) {
        return value !== tag;
      });
      setValues(updatedValues);
    } else {
      setValues(_objectSpread(_objectSpread({}, values), {}, _defineProperty({}, item, null)));
    }
  };

  // Создание отдельного массива тегов
  var tags = arr.reduce(function (acc, _ref) {
    var _ref2 = _slicedToArray(_ref, 2),
      name = _ref2[0],
      value = _ref2[1];
    if (name !== "findTags" && value && value.length !== 0) {
      var tagArr = Array.isArray(value) ? value : value.split(", ");
      return acc.concat(tagArr);
    }
    return acc;
  }, []);
  var filterCount = tags.length !== 0 ? tags : null;
  if (!filterCount) {
    return null;
  }
  var renderTags = function renderTags(item) {
    var name = item[0];
    var value = item[1];
    if (name === "findTags" || !value || value.length === 0) {
      return null;
